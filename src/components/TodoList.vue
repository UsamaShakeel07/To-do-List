...
<div class="z-10 field flex justify-center w-full items-center content-center pt-10">
        <input v-model="todo" type="text" class="rounded-l-3xl w-60 h-10 text-center justify-center text-white form-control bg-gray-800 flex z-10" placeholder="Enter Todo">    
    <button @click="submitTodo" class="rounded-r-3xl add-item h-10 w-20 justify-center bg-gray-600 text-white flex z-10"><plus-thick class="h-full"></plus-thick><p class="pt-2">Add</p>
    </button>
</div>
    ...
    <div class="py-2 inline-block min-w-full">
      <div class="overflow-hidden">
        <table class="w-full">
          ...
          <tbody>
            <tr v-for="(todo, index) in todos" :key="index" class="bg-gray-100 border-b">
              ...
              <td class="text-sm text-gray-900 font-light py-4 whitespace-nowrap">
                <div @click="editTodo(index)" class="flex z-10 justify-center"><button class="flex z-10"><square-edit-outline class="text-xl"></square-edit-outline></button></div>
              </td>
              <td class="z-10 text-sm text-gray-900 font-light py-4 whitespace-nowrap">
                <div @click="deleteTodo(index)" class="flex z-10 justify-center"><button class="flex z-10"><delete-outline class="text-xl"></delete-outline></button></div>
              </td>
            </tr>            
          </tbody>
        </table>
...
<script>
SquareEditOutline
import { PlusThick } from 'mdue';
import { SquareEditOutline } from 'mdue';
import { DeleteOutline } from 'mdue';
export default {
  name: 'TodoList',
     components: {
        PlusThick,
